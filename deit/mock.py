from torchvision.datasets.folder import (
    ...
    DatasetFolder, VisionDataset,  os, Image, make_dataset, find_classes)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        if search:
            self.alpha = nn.Parameter(torch.ones(1, 1, 1,  2*self.head_dim))
    ...
    def forward(self, x):
        if hasattr(self, 'qkv'):
            ...
            if hasattr(self, 'alpha'):
                # pdb.set_trace()
                # q_ = q_ * self.alpha[:, :, :C // self.num_heads] # torch.Size([256, 12, 197, 64])
                # k  = k *  self.alpha[:, :, C // self.num_heads: C // self.num_heads * 2]

                # print(q.size())
                # print(self.alpha.size())
                # print(self.head_dim)
                # print(self.alpha[:,:,:,:self.head_dim].size())
                # v  = v *  self.alpha[:, :, C // self.num_heads * 2 : C // self.num_heads * 3]
                #alpha1=self.alpha[:,:,:,:self.head_dim]
                q = q *(self.alpha[:,:,:,:self.head_dim]) # torch.Size([256, 12, 197, 64])
                k  = k *  ( self.alpha[:,:,:,:self.head_dim])
                v  = v *  (  self.alpha[:,:,:,self.head_dim:])
            ...
            B, N, C = x.shape # [384, 197, 768]
            q_ = self.w_q(x) # [384, 197, 600]
            k = self.w_k(x)
            v = self.w_v(x)
            # if self.w_v.weight.data.shape[0]==0:
            #     return 0
            q_ = q_.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            k  = k.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            v  = v.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)

            attn = (q_ @ k.transpose(-2, -1)) * (q_.shape[-1])**(-0.5)
            attn = attn.softmax(dim=-1) # 384, 12, 197, 197
            attn = self.attn_drop(attn)
            # q_=q_.reshape(B*self.num_heads, N,-1)
            # k=k.reshape(B*self.num_heads, N,-1)
            # v=v.reshape(B*self.num_heads, N,-1)
            # x= xops.memory_efficient_attention(q_, k, v).reshape(B,self.num_heads, N,-1).transpose(1, 2).reshape(B, N, -1)
