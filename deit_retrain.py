...
from deit.engine import train_one_epoch, evaluate, compress_model,prune_if_compressed,set_alpha_sparsity
...
def main(args, client):
    ...
    if not (args.eval or args.dist_eval):
        ...
        flops_ = FlopCountAnalysis(model, tensor)
        #print(flops_)
        print('Before prune fvcore FLOPs =', flops_.total()/1e9)
        print('Before prune FLOPs =', flops / 1e9)
        print('Before prune params =', params / 1e6)
        state_dict = torch.load(args.pruned_model_path,map_location='cpu')
        if args.resume == 'True':
            prune_if_compressed(model,state_dict)
            ...
            for name, param in model.named_parameters():
                if 'alpha' in name:
                    param.data.copy_(state_dict['model'][name])
            set_alpha_sparsity(model,sparsity=0.25)
            if args.model =='deit_base_patch16_224':
                compress_model( model,num_heads=12)
            else:
                if args.model =='deit_small_patch16_224':
                    compress_model( model,num_heads=6)
                else:
                    compress_model( model,num_heads=3)

        test_stats = evaluate(data_loader_val, model, device)
        print(f"Accuracy of the compressed network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        flops, params = profile(model, inputs=(tensor,))
        flops_ = FlopCountAnalysis(model, tensor)
        print('After prune fvcore FLOPs =', flops_.total()/1e9)
        print('After prune FLOPs =', flops / 1e9)
        print('After prune params =', params / 1e6)
